...
namespace CSharpMarkup.SystemXaml.Delegators
{
    [Bindable]
    public class BuildChild
    {
        internal static Dictionary<string, Func<CSharpMarkup.SystemXaml.UIElement>> Delegates { get; private set; } = null;

        static string Id(Delegate build) => $"{build.Method.DeclaringType.FullName}.{build.Method.Name}";

        public static string CreateIdFor(Func<CSharpMarkup.SystemXaml.UIElement> build)
        {
            AssertStateless(build);

            if (Delegates == null) Delegates = new Dictionary<string, Func<CSharpMarkup.SystemXaml.UIElement>>();
